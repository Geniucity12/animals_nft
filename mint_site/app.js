...
async function loadUserData() {
    try {
        ...
        try {
            ...
            console.log('mintedCount function not available, using balance');
    ...
    try {
        ...
        for (let tokenId = 1; tokenId <= totalSupply.toNumber() && foundTokens < balance.toNumber(); tokenId++) {
            try {
                ...
                console.log(`Token ${tokenId} doesn't exist or error:`, error.message);
...
async function createNFTCard(tokenId, container) {
    try {
        console.log(`Creating card for token ${tokenId}`);
        
        const tokenURI = await contract.tokenURI(tokenId);
        console.log(`Token ${tokenId} URI:`, tokenURI);
        
        const isUpgraded = await contract.isUpgraded(tokenId);
        console.log(`Token ${tokenId} upgraded:`, isUpgraded);
        ...
        card.innerHTML = `
            <div class="nft-image">
                <img src="${metadata.image}" alt="${metadata.name}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;" onload="console.log('Image loaded for token ${tokenId}')" onerror="this.src='data:image/svg+xml,<svg xmlns=&quot;http://www.w3.org/2000/svg&quot; width=&quot;150&quot; height=&quot;150&quot;><rect width=&quot;150&quot; height=&quot;150&quot; fill=&quot;%23f8f9fa&quot;/><text x=&quot;75&quot; y=&quot;75&quot; text-anchor=&quot;middle&quot; dy=&quot;.3em&quot; font-family=&quot;Arial&quot; font-size=&quot;14&quot; fill=&quot;%236c757d&quot;>Image Error</text></svg>'">
            </div>
        ...
        console.log(`Successfully created card for token ${tokenId}`);
...
async function mintNFT() {
    try {
        ...
        console.log('Checking mint requirements for', numTokens, 'tokens...');
        
        // Check if mint window is open
        const isMintOpen = await contract.isMintWindowOpen();
        console.log('Mint window open:', isMintOpen);
        if (!isMintOpen) {
        ...
        console.log('Total supply:', totalSupply.toString(), 'Max supply:', maxSupply.toString());
        ...
        try {
            ...
            console.log('isSaleActive function not found, continuing...');
        }
...
async function upgradeAnimal(tokenId) {
    try {
        ...
        console.log('Attempting to upgrade token:', tokenId);
        ...
        console.log('Step 1: Checking token ownership...');
        const tokenOwner = await contract.ownerOf(tokenId);
        console.log('Token owner:', tokenOwner);
        console.log('User address:', userAddress);
        console.log('Addresses match:', tokenOwner.toLowerCase() === userAddress.toLowerCase());
        ...
        console.log('Step 2: Checking if already upgraded...');
        const isAlreadyUpgraded = await contract.isUpgraded(tokenId);
        console.log('Is already upgraded:', isAlreadyUpgraded);
        ...
        console.log('Step 3: Checking canUpgrade...');
        const canUpgradeNow = await contract.canUpgrade(tokenId);
        console.log('Can upgrade now:', canUpgradeNow);
        
        if (!canUpgradeNow) {
            console.log('Step 4: Getting time until upgrade...');
            // Get time until upgrade to show helpful message
            const timeUntil = await contract.timeUntilUpgrade(tokenId);
            console.log('Time until upgrade (seconds):', timeUntil.toString());
        ...
        console.log('Step 5: All checks passed! Proceeding with upgrade...');
        showMessage('✨ Cooldown complete! Starting the magical transformation...', 'loading');

        // Proceed with upgrade
        console.log('Step 6: Calling upgradeAnimal function...');
        ...
        console.log('=== UPGRADE DEBUG END - SUCCESS ===');
